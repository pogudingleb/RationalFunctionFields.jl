...
power_sums(x, k) = [power_sum(x, i) for i = 1:k]
    ...
    for f in gb
        N, D = argmax(
            ...
            vcat(
                (N, D),
                map(
                    c -> (length(numerator(c)), length(denominator(c))),
    ...
    for f in gb
        N, D = argmax(
            ...
            vcat(
                (N, D),
                map(
                    c -> (total_degree(numerator(c)), total_degree(denominator(c))),
                    collect(coefficients(f)),
                ),
            ),
        )
...
function problems_simplification_identifiable_funcs(; with_states = true)
    ...
    for ode in odes
        ...
        ode = ode[:ode]
        id_funcs = StructuralIdentifiability.initial_identifiable_functions(
            ode,
            prob_threshold = 0.99,
...
function a_single_run_of_implementation(funcs)
    ...
    t = @elapsed RationalFunctionFields.groebner_basis_coeffs(
        rff,
        ordering = DegRevLex(),
        up_to_degree = D,
    )
...
function compute_time_per_eval(funcs)
    ...
    display(b3)
    return Dict{Any,Any}(:time_per_gb => b1, :time_per_gb_det => b2, :time_per_apply => b3)
end
...
begin
    ...
    table_6 = OrderedDict(
        k => table[k] for
        k in [:name, :input_n, :input_deg, :max_deg, :our_deg, :n_spec_sharp]
    );
    table_6[:name] = map(c -> length(c) > 40 ? c[1:40] : c, table_6[:name])
    pretty_table(
        reduce(hcat, [v for (k, v) in table_6]),
        column_labels = [
            "Example",
            "# vars.",
            "degrees (input)",
...
begin
    ...
    table_6[:name] = map(c -> length(c) > 40 ? c[1:40] : c, table_6[:name])
    table_6[:speedup] = [
        "$(round(table_6[:time_per_gb_det][i] / table_6[:time_per_apply][i], digits=2))"
        for i = 1:length(table_6[:time_per_gb_det])
    ]
    pretty_table(
        reduce(hcat, [v for (k, v) in table_6]),
        column_labels = ["Example", "F4 time", "F4 time (apply)", "Speedup"],
        fit_table_in_display_vertically = false,
    )
end
