...
method_ext = Dict("populate_data_1" => Dict("" => ".jl"))
filename_map = Dict("populate_data_1" => "run")
...
function foo()
    ...
    for system in systems
        ...
        for method in collect(templates)
            ...
            !isdir(joinpath(@__DIR__, prefix, normname(system_name))) &&
                mkdir(joinpath(@__DIR__, prefix, normname(system_name)))
            subs = ("{{name}}" => normname(system_name), "{{with_states}}" => "false")
            for (script_name, script_template) in method_scripts
                ...
                file_name =
                    string(filename_map[method_name], method_ext[method_name][script_name])
