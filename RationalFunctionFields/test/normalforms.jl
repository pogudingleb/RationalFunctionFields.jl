@testset "Linear relations over the rationals" begin
    ...
    push!(
        cases,
        Dict(
            :rff => RationalFunctionField([a, a * b + b * c]),
    ...
    for c in cases
        @test Set(RationalFunctionFields.polynomial_generators(c[:rff], c[:degree])) ==
              c[:correct]
    end
    ...
    rff = RationalFunctionField(f)
    relations = RationalFunctionFields.polynomial_generators(rff, 2)
